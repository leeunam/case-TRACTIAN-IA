...
- No grafo com planner, esse único terminal de retomada, identificado por `NON_IDEMPOTENT_OUTCOME_UNKNOWN_AFTER_RESUME`, segue diretamente de `execute_action` para `END` com mensagem determinística e sem writer/gate; qualquer outro erro ou resultado continua no writer e na porta de segurança.
