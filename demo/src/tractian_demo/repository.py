...
class DemoRepository:
        ...
        with self.transaction() as connection:
            connection.executescript(
                ...
                CREATE TABLE IF NOT EXISTS decision_requests (
                    ...
                    audience TEXT NOT NULL, kind TEXT NOT NULL, status TEXT NOT NULL,
                    subject_digest TEXT NOT NULL, scope_json TEXT NOT NULL,
                    required_permission TEXT, expires_at TEXT NOT NULL,
            ...
            for item in cases:
                connection.execute(
                    ...
                    (id,ticket_id,company_id,requester_id,asset_id,initial_message,
                     source_case_id,immutable,created_at)
                    VALUES (?,?,?,?,?,?,?,?,?)""",
                    (
                        item["id"], item["ticket_id"], item["company_id"],
                        item["user_id"], item["asset_id"], item["message"],
                        None, 1, now,
                    ),
                )
    ...
    def create_case(self, request: CreateCaseRequest) -> DemoCase:
        ...
        with self.transaction() as connection:
            if request.source_case_id is not None:
                ...
                values = (
                    case_id, source["ticket_id"], source["company_id"],
                    source["requester_id"], source["asset_id"], source["initial_message"],
                    source["id"], 0, _iso(now),
                )
            else:
                assert request.company_id and request.requester_id
                assert request.asset_id and request.message
                values = (
                    case_id, f"DEMO-{uuid4().hex[:8].upper()}", request.company_id,
                    request.requester_id, request.asset_id, request.message,
                    None, 0, _iso(now),
                )
            connection.execute(
                """INSERT INTO cases
                (id,ticket_id,company_id,requester_id,asset_id,initial_message,
                 source_case_id,immutable,created_at) VALUES (?,?,?,?,?,?,?,?,?)""",
                values,
            )
    ...
    def _case(row: sqlite3.Row) -> DemoCase:
        return DemoCase(
            id=row["id"], ticket_id=row["ticket_id"], company_id=row["company_id"],
            requester_id=row["requester_id"], asset_id=row["asset_id"],
            initial_message=row["initial_message"], source_case_id=row["source_case_id"],
            immutable=bool(row["immutable"]), created_at=_dt(row["created_at"]),
        )

    def enqueue_message(
        self, *, case_id: str, persona_id: str, content: str,
        idempotency_key: str, _fail_after_message_for_test: bool = False,
        ...
        with self.transaction() as connection:
            ...
            connection.execute(
                ...
                VALUES (?,?,?,?,?,?,?,?)""",
                (message_id, case_id, persona_id, "user", content,
                 idempotency_key, payload_hash, _iso(now)),
            )
            ...
            connection.execute(
                ...
                VALUES (?,?,?,?,0,?,?)""",
                (execution_id, case_id, message_id, ExecutionStatus.QUEUED.value,
                 _iso(now), _iso(now)),
            )
            self._append_event_tx(
                connection, case_id, "execution.queued",
                {"execution_id": execution_id, "message_id": message_id}, now,
            )
    ...
    def _message(row: sqlite3.Row) -> CaseMessage:
        return CaseMessage(
            id=row["id"], case_id=row["case_id"], persona_id=row["persona_id"],
            role=row["role"], content=row["content"], created_at=_dt(row["created_at"]),
        )
    ...
    def list_executions(self, case_id: str) -> tuple[Execution, ...]:
        rows = self.connection.execute(
            "SELECT * FROM executions WHERE case_id = ? ORDER BY created_at, id", (case_id,)
        ).fetchall()
    ...
    def _execution(row: sqlite3.Row) -> Execution:
        return Execution(
            id=row["id"], case_id=row["case_id"], message_id=row["message_id"],
            status=ExecutionStatus(row["status"]), provider=row["provider"],
            fallback_reason=row["fallback_reason"], trace_id=row["trace_id"],
    ...
    def claim_execution(self, *, worker_id: str, lease_seconds: int = 30) -> Execution | None:
        now = utc_now()
        ...
        with self.transaction() as connection:
            row = connection.execute(
                ...
                ORDER BY created_at, id LIMIT 1""", (_iso(now),)
            ).fetchone()
            if row is None:
                return None
            connection.execute(
            ...
            self._append_event_tx(connection, row["case_id"], "execution.running",
                                  {"execution_id": row["id"]}, now)
    ...
    def complete_execution(
        ...
        with self.transaction() as connection:
            ...
            connection.execute(
                ...
                VALUES (?,?,?,?,?,?)""",
                (message_id, row["case_id"], "tractian_agent", "assistant",
                 projection.assistant_message, _iso(now)),
            )
            connection.execute(
                ...
                (projection.provider, projection.fallback_reason, projection.trace_id,
                 _iso(now), execution_id),
            )
            self._append_event_tx(
                connection, row["case_id"], "tools.completed",
                {"tool_names": list(projection.tool_names)}, now,
            )
            self._append_event_tx(
                connection, row["case_id"], "agent.completed",
                {
    ...
    def fail_execution(self, execution_id: str, *, error_code: str) -> Execution:
        ...
        with self.transaction() as connection:
            ...
            self._append_event_tx(
                connection, row["case_id"], "agent.failed",
                {"execution_id": execution_id, "error_code": error_code}, now,
            )
        return self.get_execution(execution_id)

    def append_event(self, case_id: str, kind: str, payload: Mapping[str, object]) -> CaseEvent:
        now = utc_now()
        with self.transaction() as connection:
            event_id = self._append_event_tx(connection, case_id, kind, payload, now)
        row = self.connection.execute("SELECT * FROM case_events WHERE id=?", (event_id,)).fetchone()
        assert row is not None
        return self._event(row)

    @staticmethod
    def _append_event_tx(connection: sqlite3.Connection, case_id: str, kind: str,
                         payload: Mapping[str, object], now: datetime) -> int:
        ...
        cursor = connection.execute(
            ...
            (case_id, ordinal, kind, json.dumps(dict(payload), sort_keys=True), _iso(now)),
        )
    ...
    def _event(row: sqlite3.Row) -> CaseEvent:
        return CaseEvent(id=row["id"], case_id=row["case_id"], ordinal=row["ordinal"],
