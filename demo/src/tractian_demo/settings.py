...
class DemoSettings(BaseModel):
    ...
    def from_env(cls, environment: Mapping[str, str]) -> "DemoSettings":
        ...
        return cls(
            database_path=Path(environment.get("DEMO_DATABASE_PATH", cls.model_fields["database_path"].default)),
            checkpoint_path=Path(environment.get("DEMO_CHECKPOINT_PATH", cls.model_fields["checkpoint_path"].default)),
            public_cases_path=Path(environment.get("DEMO_PUBLIC_CASES_PATH", cls.model_fields["public_cases_path"].default)),
            industrial_api_url=environment.get("INDUSTRIAL_API_URL", "http://127.0.0.1:8000"),
