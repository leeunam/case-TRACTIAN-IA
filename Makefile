...
#
# Uso:
#   make setup   # cria a venv, instala a API e gera os dados
...
setup: deps data ## Cria a venv, instala dependências e gera os dados

deps: ## Cria a venv e instala dependências da API
	@command -v uv >/dev/null 2>&1 || { echo "Instale o uv: https://docs.astral.sh/uv/"; exit 1; }
	@if [ ! -x "$(PY)" ]; then cd $(ROOT)/api && uv venv --python $(PYTHON); fi
	@cd $(ROOT)/api && uv pip install --python "$(PY)" -e ".[dev]"
...
test: ## Executa os testes da API industrial
...
clean: stop clean-data ## Encerra a API e apaga dados, venv e arquivos temporários
	@rm -rf $(VENV) $(PID_DIR)
	@echo "✓ ambiente limpo"
